...
describe('geometryCentroid', () => {
  ...
  it('returns a PIP-verified point inside a U-shaped concave polygon (shoelace centroid falls in notch)', () => {
    // Shoelace centroid ≈ (28.615, 77.21) — inside the open notch, outside the polygon.
    // scanForInteriorPoint tries at 50% of height (lat≈28.62), finds spans [77.19,77.20]
    // and [77.22,77.23], verifies each candidate passes full-geometry PIP, and returns.
    const geom = uShapePolygon()
    const c = geometryCentroid(geom)
    expect(c).not.toBeNull()
    // Full-geometry PIP confirms the point is inside the ward — not in the notch
    expect(pointInGeometry(c!.lat, c!.lng, geom)).toBe(true)
    // Sanity: within the outer bounding box
  ...
  it('returns a PIP-verified point inside a polygon-with-hole (avoids landing in the hole)', () => {
    // polygonWithHole: outer square centred at (28.62, 77.21), hole centred at same point.
    // The shoelace centroid of the outer ring hits the hole, so the scanline fallback
    // must find a point that passes full-geometry PIP (outside the hole = inside the ward).
    const geom = polygonWithHole(28.62, 77.21, 0.02, 0.01)
    const c = geometryCentroid(geom)
    expect(c).not.toBeNull()
    expect(pointInGeometry(c!.lat, c!.lng, geom)).toBe(true)
